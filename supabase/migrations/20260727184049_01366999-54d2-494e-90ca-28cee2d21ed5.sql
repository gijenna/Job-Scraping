...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND COALESCE(OLD.published, false) = true
    ...
    -- already was published previously; only fire on transition to published
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS trg_notify_mn_expert_published ON public.expert_city_assignments;
CREATE TRIGGER trg_notify_mn_expert_published
BEFORE INSERT OR UPDATE OF published ON public.expert_city_assignments
FOR EACH ROW
EXECUTE FUNCTION public.notify_mn_expert_published();
