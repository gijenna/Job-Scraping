...
import {
  admin, corsHeaders, json, createSession, setSessionCookieHeader,
...
Deno.serve(async (req) => {
  ...
  if (isConsume) {
    ...
    const { data, error } = await sb
      ...
      .maybeSingle();
    if (error || !data) return json({ error: "Invalid or expired token" }, { status: 400 });
    return json({ ok: true, subject_type: data.subject_type }, { headers: setSessionCookieHeader(token) });
  }

  // ---- mint: requires admin JWT ----
  const authHeader = req.headers.get("authorization") || "";
  const jwt = authHeader.replace(/^Bearer\s+/i, "");
  if (!jwt) return json({ error: "Admin auth required" }, { status: 401 });
  ...
  if (!user || !user.email || !user.email.toLowerCase().endsWith(ADMIN_DOMAIN)) {
    return json({ error: "Admin auth required" }, { status: 403 });
  }

  const sb = admin();
  const { subject_type, lookup } = body;
  if (subject_type !== "candidate" && subject_type !== "brand_rep") {
    return json({ error: "subject_type must be 'candidate' or 'brand_rep'" }, { status: 400 });
  }
  if (!lookup) return json({ error: "lookup required (email or full name)" }, { status: 400 });
  ...
  if (!subjectId) return json({ error: "Subject not found" }, { status: 404 });
  ...
  return json({ url: url2.toString(), token });
});
