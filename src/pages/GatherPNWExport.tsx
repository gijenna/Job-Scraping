import { Zap, Star, Crown, Briefcase, MessageCircle, Mic, Check, Clock, Target, Users, Table, Flag, Plug, Wifi, Package, MapPin, MonitorSmartphone, Globe, Mail, Share2, Shield, BarChart3, HeartHandshake } from "lucide-react";
...
const tiers = [
  { name: "Hiring Table", price: "$3,000+", spots: "7 available", icon: Zap, popular: false, bestFor: "Outdoor/active lifestyle brands hiring anytime in 2026–27", perks: ["Hiring table inside event", "Up to 5 brand representatives", "Logo & careers page link on event website & registration", "Exposure to our 300K+ community"] },
  { name: "Deluxe Hiring + Branding", price: "$6,000–12,000", spots: "4 available", icon: Star, popular: true, bestFor: "Hiring AND marketing teams looking to split budget, or brands consolidating niche influence", perks: ["Everything in Hiring Table, plus:", "Much larger activation space", "Up to 20 brand representatives", "Room for product display or activation", "2-minute stage spotlight during main program", "Dedicated social post to 50K LinkedIn / 80K Facebook / 40K IG", "Free Candidate Match boost ($400) from Basecamp Match"] },
  { name: "Title Sponsorship", price: "$15,000–$25,000", spots: "1–2 available", icon: Crown, popular: false, bestFor: "Major industry players wanting top billing and full integration", perks: ['"Gather PNW 2026, Presented by [You]"', "Keynote introduction opportunity", "Prime branding at entrance, bar & stage", "Post-event engagement & lead report", "Custom in-person and/or digital activation built with your creative team"] },
];
  ...
  return (
    <div className="bg-white text-gray-900 max-w-[1200px] mx-auto" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <section className="py-16 px-8 bg-[#19363B] text-white text-center border-b-4 border-[#ED7660]">
        <img src={gatherPnwLogo} alt="Gather PNW Logo" className="w-64 mx-auto mb-6" />
        <p className="text-[#ED7660] text-xs tracking-widest uppercase mb-3">Basecamp Outdoor × University of Oregon</p>
        <p className="text-white/80 text-base max-w-xl mx-auto mb-4">
          The Pacific Northwest's premier outdoor industry career event.<br />
      ...
      <section className={sectionClass}>
        <p className={subheadClass}>Network alongside professionals from the industry's top brands</p>
      ...
      <section className={sectionClass}>
        ...
        <p className={`${bodyClass} max-w-2xl mb-6`}>Candidates worth putting in front of a hiring manager.</p>
          ...
          <div className={cardClass}>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={barData} layout="vertical" margin={{ left: 10 }}>
                ...
                <YAxis dataKey="role" type="category" width={160} tick={{ fill: "#555", fontSize: 11 }} axisLine={false} tickLine={false} />
      ...
      {/* Talent Insights */}
      <section className={sectionClass}>
        <p className={subheadClass}>Why It Matters</p>
        <h2 className={headingClass}>More Than A Career Fair</h2>
        <div className="grid grid-cols-2 gap-4 mt-4">
          {talentInsights.map((item, i) => (
            <div key={i} className="relative rounded-lg overflow-hidden min-h-[260px] border border-gray-200">
              <img src={item.image} alt="" className="absolute inset-0 w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/60 to-black/20" />
              <div className="relative z-10 flex flex-col justify-end h-full p-6">
                <p className="font-extrabold text-2xl text-[#ED7660] mb-1">{item.stat}</p>
      ...
      <section className={sectionClass}>
        ...
        <h2 className={headingClass}>The Evening</h2>
      ...
      <section className={sectionClass}>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          {[
            ...
            "Up to 5 brand reps included (more with Deluxe)", "Free appetizers and beverages",
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-2">
        ...
        <p className="text-xs text-gray-500 mt-4"><strong>Venue:</strong> U of O Portland's Campus Center at 2800 NE Liberty St, Portland, OR 97211</p>
      </section>
...
export default GatherPNWExport;
