  ...
  return (
    <div className={`w-full flex flex-col items-center justify-center py-10 select-none ${sunsetReady ? "bmp-intro-ready" : "bmp-intro-paused"}`}>
      <style>{`
        ...
        /* Kite flies off the page — starts from the top of the flutter loop with matching
           outbound velocity so there's no pause/jerk between the orbit and the exit. */
        @keyframes bmpKiteDismiss {
          0%   { opacity: 1; transform: translate(-50%, -50%) translate(0, -260px) scale(1); }
          30%  { opacity: 1; transform: translate(-50%, -50%) translate(28vw, -38vh) scale(0.92); }
          65%  { opacity: 1; transform: translate(-50%, -50%) translate(62vw, -60vh) scale(0.78); }
          100% { opacity: 0; transform: translate(-50%, -50%) translate(120vw, -94vh) scale(0.55); }
        }
        ...
        @keyframes bmpStarBurst {
          ...
          18%  { opacity: 1; }
          50%  { transform: translate(-50%, -50%) translate(var(--bx-out), var(--by-out)) scale(1) rotate(calc(var(--bspin) * 1deg)); opacity: 1; }
          100% { transform: translate(-50%, -50%) translate(0, 0) scale(0) rotate(calc(var(--bspin) * 1.6deg)); opacity: 0; }
        }
        ...
        .bmp-kite {
          ...
          animation:
            ...
            bmpKiteDismiss 2000ms linear 6500ms forwards;
        }
        .bmp-kite-wings {
          width: 100%;
          height: 100%;
