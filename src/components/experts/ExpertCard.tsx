              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span><CompanyLogoWithFallback company={expert.current_company} domainOverrides={expert.company_domains} className="w-5 h-5 bg-white" /></span>
                  </TooltipTrigger>
          ...
          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
            ...
            <TooltipProvider>
              {previousCompanies.map((company) => (
                <Tooltip key={company}>
                  <TooltipTrigger asChild>
                    <span><CompanyLogoWithFallback company={company} domainOverrides={expert.company_domains} className="w-5 h-5 bg-white/90 p-0.5" variant="secondary" /></span>
                  </TooltipTrigger>
            ...
            {expert.niche_interests && expert.niche_interests.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {expert.niche_interests.map((tag) => (
                  <Badge key={tag} variant="outline" className="text-events-cream/70 border-events-cream/20 text-xs">{tag}</Badge>
                ))}
              </div>
            )}
