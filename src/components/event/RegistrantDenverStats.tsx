...
const RainingLogo = ({ logo, leftPercent, delay, rotate }: { logo: LogoItem; leftPercent: number; delay: number; rotate: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <img src={imgSrc} alt={logo.name} className="w-5 h-5 md:w-8 md:h-8 object-contain" style={{ mixBlendMode: 'multiply' }} />
      </motion.div>
    </motion.div>
  );
};

interface RegistrantDenverStatsProps {
  logos?: LogoItem[];
}

const RegistrantDenverStats = ({ logos }: RegistrantDenverStatsProps) => {
  const allLogos = logos && logos.length > 0 ? logos : defaultLogos;
  const [testimonialIndex, setTestimonialIndex] = useState(0);

  const rainLogos = allLogos.map((logo, i) => ({
    logo,
    ...
    <section className="relative overflow-hidden" style={{ backgroundColor: "#0d1f22" }}>
      ...
      <div className="relative z-10 pb-20 md:pb-28 px-6 mt-16">
        <div className="container mx-auto max-w-2xl">
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center text-xs tracking-[0.3em] uppercase mb-10 font-body text-events-coral">
            ...
            <div className="h-[100px] flex items-center justify-center">
              ...
              >
                <div className="flex items-center justify-center gap-3">
                  <img src={`https://i.pravatar.cc/80?img=${testimonials[testimonialIndex].avatarId}`} alt="Attendee" className="w-8 h-8 rounded-full object-cover shrink-0" />
                  <p className="font-body text-sm text-events-cream/70 leading-relaxed">"{testimonials[testimonialIndex].quote}"</p>
                </div>
              </motion.div>
            </div>
            <div className="flex items-center justify-center gap-4 mt-6">
              <button onClick={() => setTestimonialIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length)} className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgba(245, 230, 211, 0.15)" }}>
                <ChevronLeft className="w-4 h-4 text-events-cream" />
              </button>
              <div className="flex gap-1.5">
                {testimonials.map((_, i) => (
                  <button key={i} onClick={() => setTestimonialIndex(i)} className="w-1.5 h-1.5 rounded-full transition-all duration-300" style={{ backgroundColor: "#F5E6D3", opacity: i === testimonialIndex ? 1 : 0.3 }} />
                ))}
              </div>
              <button onClick={() => setTestimonialIndex((prev) => (prev + 1) % testimonials.length)} className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgba(245, 230, 211, 0.15)" }}>
